...
from src.ares.behaviors.macro.auto_supply import AutoSupply
from src.ares.behaviors.macro.build_workers import BuildWorkers
from src.ares.behaviors.macro.expansion_controller import ExpansionController
from src.ares.behaviors.macro.gas_building_controller import GasBuildingController
from src.ares.behaviors.macro.macro_plan import MacroPlan
from src.ares.behaviors.macro.production_controller import ProductionController
from src.ares.behaviors.macro.spawn_controller import SpawnController
from src.ares.behaviors.macro.upgrade_controller import UpgradeController
...
class PiG_Bot(AresBot):
    ...
    async def on_step(self, iteration: int) -> None:
        ...
        worker_limit = 80
        optimal_worker_count = min(len(self.townhalls) * 22, worker_limit)
        ...
        macro_plan.add(GasBuildingController(to_count=len(self.townhalls) * 2, max_pending=2))
        macro_plan.add(ExpansionController(to_count=4, max_pending=1))

        macro_plan.add(
            ProductionController(army_composition, base_location=self.start_location, should_repower_structures=True))
        macro_plan.add(UpgradeController(desired_upgrades, base_location=self.start_location))

        macro_plan.add(SpawnController(army_composition, spawn_target=self.start_location, freeflow_mode=False))
